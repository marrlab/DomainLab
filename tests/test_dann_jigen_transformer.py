...
class VIT(nn.Module):
    ...
    def __init__(
        self, freeze=True, list_str_last_layer=["getitem_5"], len_last_layer=768
    ):
        super().__init__()
        ...
        self.features_vit_flatten = create_feature_extractor(
            self.nets, return_nodes=list_str_last_layer
        )
...
def test_transformer():
    ...
    exp = mk_exp(
        task, model, trainer="mldg_dial", test_domain="caltech", batchsize=2, nocu=True
    )
