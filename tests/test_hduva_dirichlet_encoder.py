...
def test_unit_encoder_dirichlet():
    encoder_dirichlet = EncoderH2Dirichlet(
        dim_h=64, dim_topic=3, device=torch.device("cpu"))
    feat_hidden_uniform01 = torch.rand(32, 64)  # batchsize 32
    encoder_dirichlet(feat_hidden_uniform01)
    feat_hidden_normal = torch.normal(0, 1, size=(32, 64))
