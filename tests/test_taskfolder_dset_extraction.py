...
def test_dset_sample_extraction(mode='MNIST', show_plot=False):
    ...
    if not mode == 'MNIST':
        # vlcs_mini task
        task = mk_task_folder(extensions={
            "caltech": "jpg",
            "sun": "jpg",
            "labelme": "jpg"
        },
        ...
        args = parser.parse_args([
            "--te_d", "1", "--bs", "2", "--aname", "diva", '--tpath',
            'examples/tasks/task_vlcs.py', '--nname', 'conv_bn_pool_2'
        ])
        dset_name = 'vlcs_mini'
    else:
        # MNISTColor10 task
        ...
        # MNISTColor10 args
        args = parser.parse_args([
            "--te_d", "1", "--bs", "4", "--aname", "diva", '--task',
            'mnistcolor10', '--nname', 'conv_bn_pool_2'
        ])
    ...
    for domain in task.get_list_domains():
        ...
        for class_num in range(len(task.list_str_y)):
            num_of_samples = 0
            loader_domain = data_utils.DataLoader(d_dataset,
                                                  batch_size=1,
                                                  shuffle=False)
            domain_targets = []
            image_list = []
            ...
            if show_plot:
                ...
                for i in range(sample_num):
                    ...
                    ax[i].set_title(
                        ...
                        ', label: ' + str(task.list_str_y[np.argmax(
                            np.array(label_list[i]))]))
                plt.suptitle(
                    'dataloader task.get_dset_by_domain(args, domain)')
                plt.tight_layout()
                plt.show()

            class_dataset = Subset(d_dataset, domain_targets)
            full_f_name = f_name + '/' + str(domain) + '/' + str(
                task.list_str_y[class_num]) + '.jpg'
