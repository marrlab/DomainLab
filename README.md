...
```
where `--tpath` specifies the path of a user specified python file which defines the domain generalization task, see Example in [Task Specification](domainlab/docs/html/doc_tasks.html). `--aname` specifies which algorithm to use, see [Available Algorithms](domainlab/docs/html/doc_algos.html), `--bs` specifies the batch size, `--debug` restrain only running for 2 epochs and save results with prefix 'debug'. For DIVA, the hyper-parameters include `--gamma_y=7e5` which is the relative weight of ERM loss compared to ELBO loss, and  `--gamma_d=1e5`, which is the relative weight of domain classification loss compared to ELBO loss.
...
[Task Specification](domainlab/docs/html/doc_tasks.html) 
...
[Specify Custom Neural Networks for an algorithm](domainlab/docs/html/doc_custom_nn.html) 
